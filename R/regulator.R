...
regulator<-function(target,data,plot=TRUE,type="1",scale=TRUE,group='med',FC=2,FDR=0.05){
  ...
  if(type==1){
    ...
    if(length(high)<3 | length(low)<3){
      stop("The number of samples must more than 3 in every group !")
      ...
      for(i in 1:n){
        if(scale){
          ...
          p<-t.test(as.numeric(data[i,high]),as.numeric(data[i,low]),var=TRUE)$p.value
  ...
  if(type==2){
    ...
    for(i in 1:n){
      ...
      if(scale){
        ...
        p<-t.test(as.numeric(tdata[high]),as.numeric(tdata[low]),var=TRUE)$p.value
